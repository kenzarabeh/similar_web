...
class SimilarWebAPI:
    ...
    def get_segment_data(self, segment_id: str, start_date: str, end_date: str, 
        ...
        """
        from config import SEGMENT_METRICS_GROUPS
        ...
        for metrics_group in SEGMENT_METRICS_GROUPS:
            ...
            if result and 'segments' in result:
                segment_data = result['segments'][0]
                all_segments_data.append(segment_data)
                logger.info(f"    ✅ Métriques récupérées - Clés: {list(segment_data.keys())}")
        ...
        if all_segments_data:
            ...
            # Logger toutes les clés disponibles après la combinaison
            logger.info(f"📊 Métriques combinées pour le segment: {list(combined_segment.keys())}")
...
def save_results_to_json(data: Any, filename: str) -> None:
    ...
    # En Cloud Functions, on ne peut écrire que dans /tmp
